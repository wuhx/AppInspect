...
//     }
//
//     pub fn update_cache(&mut self, pi: &pb::PackageInfo) -> anyhow::Result<()> {
...
//         resp.encode(&mut buf)?;
//         self.backend.insert("pkg_list", buf)?;
//         self.backend.flush()?;
...
//         self.update_cache_batch(&resp)?;
//         self.update_local_cache(&resp)?;
//         Ok(())
//     }
...
//                 let _ = self.update_cache_batch(&resp);
//                 Ok(())
//             }
//             Ok(None) => {
...
//             let _ = self.cache_provisioning_rpc();
//             return;
//         }
//     }
//
